...
namespace ChocDino.UIFX
{
	...
	public class OutlineFilter : FilterBase
	{
		...
		[Tooltip("The first color of the outline's gradient, or just the default color if not using gradient.")]
		[FormerlySerializedAs("_color")]
		[SerializeField] Color _color1 = Color.black;
		
		[Tooltip("The second color of the outline's gradient.")]
		[SerializeField] Color _color2 = Color.black;

		[FormerlySerializedAs("useGradient")]
		[Tooltip("If checked, interpolates between the first and second color using the specified gradient mode")]
		[SerializeField] bool _useGradient;

		[FormerlySerializedAs("gradientType")]
		[Tooltip("The type of gradient to use")]
		[SerializeField] GradientType _gradientType;

		[Tooltip("The offset of the outline")] 
		...
		static new class ShaderProp
		{
			...
			public readonly static int OutlineColor1 = Shader.PropertyToID("_OutlineColor1");
			public readonly static int OutlineColor2 = Shader.PropertyToID("_OutlineColor2");
			public readonly static int GradientType = Shader.PropertyToID("_GradientDirection");
			public readonly static int Offset = Shader.PropertyToID("_Offset");
		...
		protected override void SetupDisplayMaterial(Texture source, Texture result)
		{
			if (this.Strength >= 1f)
			{
				...
				_displayMaterial.SetColor(ShaderProp.OutlineColor1, _color1);
				if (_useGradient)
				{
					_displayMaterial.SetColor(ShaderProp.OutlineColor2, _color2);
				}
				else
				{
					_displayMaterial.SetColor(ShaderProp.OutlineColor2, _color1);
				}
				
				_displayMaterial.SetInt(ShaderProp.GradientType, (int)_gradientType);
				
				_displayMaterial.SetVector(ShaderProp.Offset, new Vector4(_offset.x, _offset.y, 0, 0));
			}
			else
			{
