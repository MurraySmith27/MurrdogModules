Shader "Hidden/ChocDino/UIFX/Blend-Outline"
{
	Properties
	{
		...
		_OutlineColor1 ("Outline Color", Vector) = (1, 0, 0, 1)
		_OutlineColor2 ("Outline Color", Vector) = (1, 0, 0, 1)
		_GradientDirection ("Gradient Direction", Int) = 0 //0 for vertical, 1 for horizontal
	...
	uniform int _GradientDirection;
	...
	v2fOutline vertOutline (appdata v)
	{
		...
		//offset the vertices in clip space
		// o.vertex = o.vertex + float4(_Offset.x, -_Offset.y, 0, 0);
	...
	float4 fragOutline(v2fOutline i) : SV_Target
	{
		...
		float4 outlineColor1 = ToPremultiplied(_OutlineColor1);
		float4 outlineColor2 = ToPremultiplied(_OutlineColor2);
		#else
		float4 outlineColor1 = StraightGammaToPremultipliedLinear(_OutlineColor1);
		float4 outlineColor2 = StraightGammaToPremultipliedLinear(_OutlineColor2); 
		#endif

		float4 verticalGradientOutlineColor = lerp(outlineColor1, outlineColor2, i.uv.x);
		float4 horizontalGradientOutlineColor = lerp(outlineColor1, outlineColor2, i.uv.y);

		float4 outlineColor = (horizontalGradientOutlineColor * _GradientDirection) + (verticalGradientOutlineColor * (1 - _GradientDirection));
		...
		#ifdef DISTANCEMAP
		{
			float distance = tex2D(_ResultTex, i.uv.xy + float2(_Offset.x, -_Offset.y)).x;
		...
		#else
		{
			outlineMask = tex2D(_ResultTex, i.uv.xy + float2(_Offset.x, -_Offset.y)).x;
