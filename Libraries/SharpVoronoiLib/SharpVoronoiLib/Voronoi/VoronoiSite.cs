...
namespace SharpVoronoiLib
{
    ...
    {
        [PublicAPI] public double X { get; private set; }

        [PublicAPI] public double Y { get; private set; }
        ...
        public IReadOnlyList<VoronoiPoint> Points
        {
            get
            {
                ...
                if (_points == null)
                {
                    _points = new List<VoronoiPoint>(edges
                        .Count); // afaik only unclosed cells will have more points than edges
        ...
        public bool Contains(double x, double y)
        {
            ...
            {
                if (_clockwisePoints[i].Y < y && _clockwisePoints[j].Y >= y ||
                    _clockwisePoints[j].Y < y && _clockwisePoints[i].Y >= y)
                {
                    if (_clockwisePoints[i].X + ((y - _clockwisePoints[i].Y) /
                                                 (_clockwisePoints[j].Y - _clockwisePoints[i].Y) *
                                                 (_clockwisePoints[j].X - _clockwisePoints[i].X)) < x)
                    {
                        result = !result;
                    }
                }
        ...
        private const double
            shiftAmount =
                1 / 1E14; // the point of shifting coordinates is to "change the angle", but Atan cannot distinguish anything smaller than something like double significant digits, so we need this "epsilon" to be fairly large
