...
namespace SharpVoronoiLib
{
    ...
    public class VoronoiEdge : IEquatable<VoronoiEdge>
    {
        ...
        public IReadOnlyList<VoronoiEdge> Neighbours
        {
            get
            {
                if (_neighbours == null)
                {
                    ...
                    bool GatherNeighbours(VoronoiPoint aroundPoint, VoronoiEdge comingFromEdge,
                        VoronoiSite comingIntoSite, VoronoiSite? ultimateSite)
                    {
                        foreach (VoronoiEdge edge in comingIntoSite.Edges)
                        {
                                ...
                                edge.End == aroundPoint)
                            {
                                ...
                                {
                                    if (ultimateSite !=
                                        null && // the other side of the original edge could have been a border/void
                                        nextSite ==
                                        ultimateSite) // we reached the other side of the original edge, so no more edges remain around this point
                            ...
                            {
                                if (edge.Start.ApproxEqual(aroundPoint))
                                    throw new InvalidOperationException("Equal but not same " + edge.Start + " and " +
                                                                        aroundPoint);
                                if (edge.End.ApproxEqual(aroundPoint))
                                    throw new InvalidOperationException("Equal but not same " + edge.End + " and " +
                                                                        aroundPoint);
                            }
                        }
        ...
        internal VoronoiEdge?
            LastBeachLineNeighbor
        {
            get;
            set;
        } // I am not entirely sure this is the right name for this, but I just want to make it clear it's not something usable publicly
        ...
        public override int GetHashCode()
        {
            ...
            unchecked
            {
                return (sx.GetHashCode() * 397) ^ (sy.GetHashCode() * 397) ^ (ex.GetHashCode() * 397) ^
                       ey.GetHashCode();
            }
        }
