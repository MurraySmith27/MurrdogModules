...
namespace SharpVoronoiLib
{
    ...
    internal class GenericClipping : IBorderClippingAlgorithm
    {
        ...
        /// </summary>
        [MustUseReturnValue]
        private static bool ClipEdge(VoronoiEdge edge, bool asNeighbour, double minX, double minY, double maxX,
            double maxY)
        {
            bool accept = false;
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    {
                        x = edge.Start.X + (edge.End.X - edge.Start.X) * (maxY - edge.Start.Y) /
                            (edge.End.Y - edge.Start.Y);
                        y = maxY;
                    }
                    else if (outcode.HasFlag(Outcode.Bottom))
                    {
                        x = edge.Start.X + (edge.End.X - edge.Start.X) * (minY - edge.Start.Y) /
                            (edge.End.Y - edge.Start.Y);
                        y = minY;
                    }
                    else if (outcode.HasFlag(Outcode.Right))
                    {
                        y = edge.Start.Y + (edge.End.Y - edge.Start.Y) * (maxX - edge.Start.X) /
                            (edge.End.X - edge.Start.X);
                        x = maxX;
                    }
                    else if (outcode.HasFlag(Outcode.Left))
                    {
                        y = edge.Start.Y + (edge.End.Y - edge.Start.Y) * (minX - edge.Start.X) /
                            (edge.End.X - edge.Start.X);
                        x = minX;
                    }

                    VoronoiPoint finalPoint = new VoronoiPoint(x, y,
        ...
        [Pure]
        private static bool ClipRay(VoronoiEdge edge, bool asNeighbour, double minX, double minY, double maxX,
            double maxY)
        {
                ...
                if (Within(start.X, minX, maxX))
                {
                    VoronoiPoint endPoint =
                        edge.SlopeRun.ApproxGreaterThan(0)
                            ...
                            : // never BottomRight or TopRight
                            new VoronoiPoint(minX, start.Y,
                                start.Y.ApproxEqual(minY) ? PointBorderLocation.BottomLeft :
                                start.Y.ApproxEqual(maxY) ? PointBorderLocation.TopLeft : PointBorderLocation.Left);
                ...
                if (Within(start.Y, minY, maxY))
                {
                    VoronoiPoint endPoint =
                        edge.SlopeRise.ApproxGreaterThan(0)
                            ? new VoronoiPoint(start.X, maxY,
                                start.X.ApproxEqual(minX) ? PointBorderLocation.TopLeft :
                                start.X.ApproxEqual(maxX) ? PointBorderLocation.TopRight : PointBorderLocation.Top)
                            : new VoronoiPoint(start.X, minY,
                                PointBorderLocation.Bottom); // never BottomLeft or BottomRight
            ...
            VoronoiPoint topX = new VoronoiPoint(topXValue, maxY,
                topXValue.ApproxEqual(minX) ? PointBorderLocation.TopLeft :
                topXValue.ApproxEqual(maxX) ? PointBorderLocation.TopRight : PointBorderLocation.Top);

            double rightYValue = CalcY(edge.Slope.Value, maxX, edge.Intercept.Value);
            VoronoiPoint rightY = new VoronoiPoint(maxX, rightYValue,
                rightYValue.ApproxEqual(minY) ? PointBorderLocation.BottomRight :
                rightYValue.ApproxEqual(maxY) ? PointBorderLocation.TopRight : PointBorderLocation.Right);

            double bottomXValue = CalcX(edge.Slope.Value, minY, edge.Intercept.Value);
            VoronoiPoint bottomX = new VoronoiPoint(bottomXValue, minY,
                bottomXValue.ApproxEqual(minX) ? PointBorderLocation.BottomLeft :
                bottomXValue.ApproxEqual(maxX) ? PointBorderLocation.BottomRight : PointBorderLocation.Bottom);

            double leftYValue = CalcY(edge.Slope.Value, minX, edge.Intercept.Value);
            VoronoiPoint leftY = new VoronoiPoint(minX, leftYValue,
                leftYValue.ApproxEqual(minY) ? PointBorderLocation.BottomLeft :
                leftYValue.ApproxEqual(maxY) ? PointBorderLocation.TopLeft : PointBorderLocation.Left);
            ...
            List<VoronoiPoint> candidates = new();
            ...
            if (candidates.Count == 2)
            {
                ...
                if ((ax * ax + ay * ay).ApproxGreaterThan(bx * bx + by * by))
                {
                    ...
                    edge.Start.BorderLocation =
                        GetBorderLocationForCoordinate(edge.Start.X, edge.Start.Y, minX, minY, maxX, maxY);
            ...
            static bool Within(double x, double a, double b) =>
                x.ApproxGreaterThanOrEqualTo(a) && x.ApproxLessThanOrEqualTo(b);
        ...
        [Pure]
        private static PointBorderLocation GetBorderLocationForCoordinate(double x, double y, double minX, double minY,
